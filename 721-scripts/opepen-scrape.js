...
const artistName = "Jack Butcher";
const searchQuery = "IX"
const searchProperty = "Opepen";
...
async function main() {
    ...
    if (artistIDQuery.results[0]) {
        ...
        artList.forEach(artname => {
            ...
            setTimeout(() => {
                addItem(`${artname} - ${searchQuery}`, artStorage[artname][0].tokenType.slice(3), artStorage[artname][0].contract.openSea.collectionName, artistNotionID, contractAddress, newIDs, artType);
            }, 5000);
        });
    }
}
